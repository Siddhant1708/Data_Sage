...
def print_research_response(response_data):
    print("\n" + "=" * 50)
    print(f"📌  Topic: {response_data.topic}\n")
    print(f"📖  Summary:\n{response_data.summary}\n")
    print(f"🔗  Sources: {', '.join(response_data.sources)}")
    print(f"🛠️  Tools Used: {', '.join(response_data.tools_used)}")
    print("=" * 50 + "\n")


try:
    parsed_output = json.loads(raw_response["output"])  # Convert string to dict
    structured_response = ResearchResponse(**parsed_output)  # Convert to Pydantic object
    
    # Print in a formatted way
    print_research_response(structured_response)
